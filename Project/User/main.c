/**
******************************************************************************
* @file    \StandAlone\Project\User\main.c 
...
int main(void)
{
  /* Inicializações / Initializations */
  GPIO_Config();
  TIM4_Config();
  TIM3_Config();
  ...
  while(1)
  {
    ...
    {
      // Rotina de gravação do STM8 / STM8 Flashing routine
      STM8_program();
...
{
  TIM_Cmd(TIM3, ENABLE);
  GPIO_WriteBit(GPIOA, GPIO_Pin_8, (BitAction)(Bit_SET));//LED ON
  delay_ms(50);
  GPIO_WriteBit(GPIOA, GPIO_Pin_8, (BitAction)(Bit_RESET));//LED OFF
  delay_ms(50);
  GPIO_WriteBit(GPIOA, GPIO_Pin_8, (BitAction)(Bit_SET));//LED ON
  delay_ms(50);
  GPIO_WriteBit(GPIOA, GPIO_Pin_8, (BitAction)(Bit_RESET));//LED OFF
